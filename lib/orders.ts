...
]);const {results:columns}=await db.prepare("PRAGMA table_info(orders)").all<{name:string}>();if(!columns.some(column=>column.name==="discord_username"))await db.prepare("ALTER TABLE orders ADD COLUMN discord_username TEXT NOT NULL DEFAULT ''").run();if(!columns.some(column=>column.name==="discount_code"))await db.prepare("ALTER TABLE orders ADD COLUMN discount_code TEXT NOT NULL DEFAULT ''").run();if(!columns.some(column=>column.name==="discount_cents"))await db.prepare("ALTER TABLE orders ADD COLUMN discount_cents INTEGER NOT NULL DEFAULT 0").run()}
...
export async function publicReviews(){const db=await getDb();const {results}=await db.prepare("SELECT o.items_json,e.body,e.created_at FROM order_events e JOIN orders o ON o.id=e.order_id WHERE e.type='vouch' AND e.body!='' ORDER BY e.created_at DESC LIMIT 12").all<Record<string,unknown>>();return results.flatMap(row=>{try{const body=JSON.parse(String(row.body)) as {rating?:number;review?:string};const product=(JSON.parse(String(row.items_json)) as OrderItem[])[0]?.name||"Digital product";return Number.isInteger(body.rating)&&body.review?[{product,rating:Number(body.rating),review:String(body.review),createdAt:String(row.created_at)} satisfies PublicReview]:[]}catch{return[]}})}

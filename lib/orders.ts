...
const encodeBase32=(buffer:Buffer)=>{const alphabet="ABCDEFGHIJKLMNOPQRSTUVWXYZ234567";let bits="";for(const byte of buffer)bits+=byte.toString(2).padStart(8,"0");let output="";for(let i=0;i<bits.length;i+=5)output+=alphabet[parseInt(bits.slice(i,i+5).padEnd(5,"0"),2)];return output};
async function setting(key:string){const db=await getDb();return(await db.prepare("SELECT value FROM settings WHERE key=?").bind(key).first<{value:string}>())?.value||""}
async function saveSetting(key:string,value:string){const db=await getDb();await db.prepare("INSERT INTO settings (key,value,updated_at) VALUES (?,?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=excluded.updated_at").bind(key,value,new Date().toISOString()).run()}
export async function activeTwoFactorSecret(){return process.env.OWNER_TOTP_SECRET||await setting("owner_totp_secret")}
export async function beginTwoFactor(){const secret=encodeBase32(randomBytes(20));await saveSetting("owner_totp_pending",secret);return{secret,uri:`otpauth://totp/FOREVERREPENT:Owner?secret=${secret}&issuer=FOREVERREPENT&algorithm=SHA1&digits=6&period=30`}}
export async function pendingTwoFactorSecret(){return setting("owner_totp_pending")}
export async function confirmTwoFactor(){const secret=await pendingTwoFactorSecret();if(!secret)throw new Error("Start 2FA setup first.");await saveSetting("owner_totp_secret",secret);const db=await getDb();await db.prepare("DELETE FROM settings WHERE key='owner_totp_pending'").run()}
export async function disableTwoFactor(){const db=await getDb();await db.prepare("DELETE FROM settings WHERE key IN ('owner_totp_secret','owner_totp_pending')").run()}
export async function databaseBackup(){const db=await getDb();const tables=["orders","messages","order_events","discount_codes","settings"];const data:Record<string,unknown[]>={};for(const table of tables){try{const rows=(await db.prepare(`SELECT * FROM ${table}`).all<Record<string,unknown>>()).results;data[table]=table==="settings"?rows.filter(row=>!String(row.key).startsWith("owner_totp")):rows}catch{data[table]=[]}}return{version:1,exportedAt:new Date().toISOString(),data}}

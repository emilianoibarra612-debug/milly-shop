...
export async function recentSales(){const db=await getDb();const {results}=await db.prepare("SELECT items_json,updated_at FROM orders WHERE status='completed' ORDER BY updated_at DESC LIMIT 6").all<Record<string,unknown>>();return results.map(row=>({product:(JSON.parse(String(row.items_json)) as OrderItem[])[0]?.name||"Digital product",completedAt:String(row.updated_at)}))}

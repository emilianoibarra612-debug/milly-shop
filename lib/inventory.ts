...
export function validTotp(code:string,secret=process.env.OWNER_TOTP_SECRET||""){if(!secret)return true;if(!/^\d{6}$/.test(code))return false;const key=decodeBase32(secret);const now=Math.floor(Date.now()/30000);for(let offset=-1;offset<=1;offset++){const counter=Buffer.alloc(8);counter.writeBigUInt64BE(BigInt(now+offset));const digest=createHmac("sha1",key).update(counter).digest();const start=digest[digest.length-1]&15;const value=((digest.readUInt32BE(start)&0x7fffffff)%1000000).toString().padStart(6,"0");if(safeEqual(value,code))return true}return false}

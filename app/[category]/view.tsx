...
type Store={productEdits:Record<string,{name:string;description:string}>;customProducts:(Product&{category:string})[]};
export default function CategoryClient({category}:{category:Category}){
 const [stock,setStock]=useState<Record<string,number>>({});const [store,setStore]=useState<Store>({productEdits:{},customProducts:[]});
 ...
 const products=[...category.products,...store.customProducts.filter(product=>product.category===category.slug)].map(product=>({...product,...store.productEdits?.[key(category.slug,product.slug)]}));

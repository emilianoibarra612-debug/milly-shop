...
import { makeSession, twoFactorEnabled, validLogin, validTotp } from "@/lib/inventory";
import { loginAllowed, recordLogin } from "@/lib/orders";
export const runtime = "nodejs";
export async function GET(){return NextResponse.json({twoFactorEnabled:twoFactorEnabled()},{headers:{"Cache-Control":"no-store"}})}
export async function POST(request: Request) {
  ...
  if(!validTotp(String(otp||""))){await recordLogin(ip,false);return NextResponse.json({error:"Invalid authenticator code."},{status:401})}
